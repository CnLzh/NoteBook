...
1. 红黑树：用于管理存放事件的集合，这些事件以`epitem`作为节点挂载到红黑树上，并与设备驱动建立回调关系。其主要需求为在大量并发的场景下，快速的添加、修改、删除节点。选择红黑树的数据结构解决该问题，每次通过`epoll_ctl`的任何操作的时间复杂度为 $O(\log N)$。
...
2. 事件以节点的形式挂载到红黑树上，通过`epoll_ctl`的任何操作的事件复杂度都是 $O(\log N)$ 。
3. `epoll_wait`仅关注就绪队列是否为空，不需要遍历事件集合。
4. 向内核中断注册回调函数，一旦有事件触发，由回调将事件对应节点添加到就绪队列中，时间复杂度仅为 $O(1)$ 。
